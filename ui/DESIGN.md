...
**The pot leads the felt.** It is the single most-read number on the play surface, so it is the largest number on it — but leading is one clear step above its neighbours, not the top of the type scale. `--font-size-title` (24px) on the oval, one step down (`--font-size-large`, then `--font-size-subtitle`) as the ring shrinks. At `--font-size-xl` (32px) it read as oversized next to the 68px board and the ~11px seat text beside it; `title` keeps it dominant and in proportion. Its chip stack is `--chip-size: 18px` (`.chip-stack-pot`), sized to sit with the figure rather than tower over it. The word "POTE" is a quiet mono marker beside it, baseline-aligned, never its equal.
...
**Entrance animations are never what makes something visible.** A `seat-join`-style fade-in starts at `opacity: 0` and holds it (`animation-fill-mode: both`) until the animation actually advances, so any element whose resting state comes from an entrance animation renders as nothing in a browser that has not started ticking its rendering loop. WebKit does exactly that — a pending CSS animation sits at `startTime: null`, held at time 0, indefinitely — which is how every seat on the table came to render invisible and 12px off its orbit point in Safari while Chromium and Firefox looked correct. So an entrance animation is applied only to the element that actually just arrived, marked for exactly one animation duration (`data-seat-joining` / `data-seat-leaving`, from `useJoinedSeats` / `useDepartedSeats`), never to every member of a collection on every mount. What is simply *there* wears its stylesheet's resting state and nothing else.
...
- **Don't** let an entrance animation produce a resting state. Animate what just arrived, for one animation; an element that fails to animate must still render.
