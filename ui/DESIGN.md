...
**Felt wordmark.** The `{PokerLogo} CTECH` house mark flies in the top arc of the felt on both stages (and in replay), matching the landing hero's table preview. It is decorative (`aria-hidden`; the stage name lives on the page `h1`) but not a watermark: the `CTECH` lettering is `on-brand` **white** and the P monogram keeps its oxblood, so the table reads as CTech's from across the room. White, never gold — the Three Materials Rule reserves gold for value, time, and earned outcomes, and a gold mark on the play surface would be decoration wearing value's colour. The whole lockup scales from one local token (`--felt-wordmark-mark`, 26px on the oval, 19px on the portrait capsule). It never moves and it carries no filter.

**The felt's three bands.** Everything on the felt owns a horizontal band and nothing crosses another's. Top: the wordmark. Middle-top: the transient dealer call (`.table-callout`). Centre: the board with the street rail hung directly beneath it (`.felt-center`), never pinned to the felt's bottom edge — that lower band belongs to the bottom-row seats' bet chips, which travel up toward the pot. Anything new on the felt claims a band or attaches to one of these, never a fresh percentage.
