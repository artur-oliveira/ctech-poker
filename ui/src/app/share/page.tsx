...
function SharedHandContent() {
  ...
  if (share.isLoading || !share.data) return <section className="public-hand loading"><span className="loader"/>Carregando mão compartilhada…</section>;
    ...
    <div className="public-hand-cards">
      <article>
        ...
        <div>{item.hero_cards?.length ? item.hero_cards.map((card, index) =>
          <PlayingCard key={card} card={card} index={index} size="hole" owner="viewer"/>) : <span>Cartas ocultas</span>}</div>
      </article>
      <article>
