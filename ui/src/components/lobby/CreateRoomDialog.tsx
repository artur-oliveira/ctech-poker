...
export function CreateRoomDialog() {
  ...
  async function submit(values: Values) {
    ...
    try {
      const room = await createRoom({visibility: values.visibility, small_blind: stake.small_blind, big_blind: stake.big_blind, max_seats: values.seats, buy_in_min: stake.big_blind * 20, buy_in_max: stake.big_blind * 100})
  ...
  return <Dialog open={open} onOpenChange={setOpen}>
    ...
    <DialogContent>
      <DialogHeader><p className="font-mono text-xs tracking-widest text-(--brand-bright)">NOVA MESA</p><DialogTitle>Configure sua mesa</DialogTitle><DialogDescription>Escolha como quer jogar. Os valores abaixo são fichas virtuais do sandbox.</DialogDescription></DialogHeader>
