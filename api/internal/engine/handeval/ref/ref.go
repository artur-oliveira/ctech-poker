// Package ref is the reference (slow, obviously-correct) hand evaluator.
//
...
// BestN returns the highest Score achievable from any 5 of n cards (5 <= n <= 7).
func BestN(cards []deck.Card) Score {
	n := len(cards)
	var best Score
	idx := [5]int{0, 1, 2, 3, 4}
	for {
