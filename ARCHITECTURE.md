...
30 DynamoDB tables, all `TableV2`, on-demand billing, name-prefixed `<env>_`, defined in
`cdk/lib/dynamodb-stack.ts` (see `cdk/CLAUDE.md` for the current authoritative count/history —
it has tracked this number as features shipped; this section only lagged behind it). Partition
key is always `pk` (S), sort key `sk` (S) where present. This section originally documented the
first 15; the 15 added since — cosmetics, reaction purchases, pay-to-see-cards/rabbit-hunt
reveals, matchup history, table highlights and the social graph — are listed in the "Monetized
extras" and "Social graph" groups below.
...
**Monetized extras (cosmetics, reactions, pay-to-see-cards)**

- `poker_sandbox_purchases`, `poker_reaction_purchases`, `poker_cosmetic_purchases` — purchase
  ledgers for sandbox-chip packs, premium reactions and cosmetics, each paired with an
  entitlements table below.
- `poker_reaction_entitlements`, `poker_cosmetic_entitlements`, `poker_table_entitlements` — what
  a player (or a table, for table-level cosmetics/entitlements) currently owns/has unlocked,
  read on the hot path instead of re-deriving from the purchase ledger.
- `poker_hand_reveals`, `poker_hand_reveal_payments` — the paid rabbit-hunt / pay-to-see-cards
  flow: a reveal request and its associated payment record, kept separate from the free
  `poker_hand_shares` proof.
- `poker_hand_meta`, `poker_player_matchups` — supplementary hand metadata and head-to-head
  matchup stats, split out from `poker_player_hands`/`poker_player_poker_stats` rather than
  overloading those items.
- `poker_table_highlights` — the table highlights feed (notable hands/moments surfaced in the
  lobby).

**Social graph** (`SOCIAL_GRAPH_ENABLED`-gated, `docs/plans/2026-08-16-social-friends-safety-and-recent.md`)

- `poker_social_edges`, `poker_social_events` — mutual friendship edges and the friend/invite
  activity feed.
- `poker_recent_players` — recently-played-with list (for the "add friend" flow).
- `poker_player_reports` — safety/moderation reports; independent of `SOCIAL_GRAPH_ENABLED`.
...
- Frontend hardening is applied at the Cloudflare edge (the frontend moved off CloudFront/S3 to
  Cloudflare Pages — see `docs/plans/` in `ctech-cdk` for the migration): CSP with
  `default-src 'self'`, `frame-ancestors 'none'`, `object-src 'none'`, `X-Frame-Options: DENY`
  and an optional `Permissions-Policy` header, generated by the shared reusable workflow
  `ctech-cdk/.github/workflows/frontend-cloudflare.yml` that `.github/workflows/frontend.yml`
  calls on deploy. There is no `cdk/lib/frontend-stack.ts` in this repo — this repo's `cdk/`
  only provisions the API/backend stacks (see `cdk/lib/`).
