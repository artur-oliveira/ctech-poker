...
class PublicProfile extends StatelessWidget {
  ...
  Widget build(BuildContext context) => Scaffold(
    ...
    body: SafeArea(
      child: AsyncPanel(
        load: () => Future.wait([
          api.get('/v1.0/players/${segment(playerId)}/showcase'),
          api.get('/v1.0/players/me/matchups/${segment(playerId)}'),
        ]),
        builder: (context, results, _) {
          final profile = results[0] as Json, matchup = results[1] as Json;
          return ListView(
            ...
            children: [
              ...
              for (final achievement in rows(profile, 'featured_achievements'))
                ListTile(
              ...
              if (profile['best_hand'] != null &&
                  !(profile['showcase_layout']?['hidden'] as List? ?? [])
                      .contains('best_hand'))
                Card(
                  child: Padding(
              ...
              if (!(profile['showcase_layout']?['hidden'] as List? ?? [])
                  .contains('matchup')) ...[
                const SizedBox(height: 20),
                Text(
                  'Nosso confronto',
