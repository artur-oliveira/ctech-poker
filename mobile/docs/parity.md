...
| Mesa | Protobuf, snapshots versionados, ações legais, aumento, pré-seleções, pausa, saída/cancelamento, chat/moderação, reações, recompra, cartas, dois boards, resultados | Controles completos de auto-rebuy, handoff entre dispositivos, cenários de duração longa |
...
| Jornada | Estatísticas, conquistas, ranking, sessões | Nomes editoriais de conquistas, destaques, ranking pessoal, todos os filtros e modos |
...
| Distribuição | Workflow Android + macOS, APK debug, simulador, compile iOS sem assinatura | Execução remota; assinatura Android/iOS; TestFlight; validação em aparelhos |
...
A integração complementar no Accounts passou em `go test ./internal/domain/oauth/client ./internal/handler -race`; a API Poker passou em `go test ./... -race`. Os 19 testes Flutter passaram, incluindo a referência visual portrait. A análise estática não apontou problemas. Respostas HTTP de sucesso malformadas são rejeitadas; falhas ao carregar filtros salvos impedem sobrescrever a lista desconhecida.
